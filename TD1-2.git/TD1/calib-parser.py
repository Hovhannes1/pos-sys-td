...
class RSSIRecord():
    ...
    '''
    def addSample(self, macaddr, rssi):
        self.sample.append(RSSISample(macaddr,rssi))
      ...
      with open(file_name, 'r') as file:
        ...
        for row in my_reader:
          ...
          for x in range(4,len(row),2):
              while j < len(self.database[i].samples ) and not exist:
                  if (self.database[i].samples[j].macaddr == row[x]):
                      exist=True
              ...
              if (not exist):
                  if first:
                      self.database[i].samples.append(RSSISample(row[x]))
                      first = False
                  else:
                      index=0
                      for k in range(0,len(self.database[i].samples)+1,1):
                          if(str(self.database[i].samples[k-1])>=row[x]):
                              index = k
                      self.database[i].samples.insert(index,RSSISample(row[x]))
                              
              self.database[i].samples[j].rssi.append(row[x+1])
              exist = False
              j = 0
          first = True
          count=count+1
          if(count==4):
              print(str("x:"+self.database[i].x)+" y:"+str(self.database[i].y)+" z:"
                    +str(self.database[i].z)+" orientation:"+str(self.database[i].orientation))
              for k in range(0,len(self.database[i].samples),1):
                  print(str(self.database[i].samples[k].macaddr)
                        +" "+str(self.database[i].samples[k].computeAverageValue()))
              count = 0
