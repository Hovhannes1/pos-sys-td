...
class RSSISample():
    ...
    def computeAverageValue(self):
        ...
        return 10*log10(sum_mw/len(self.rssi))
...
class RSSIDatabase():
    ...
    '''
    def checkExistance(self,x,y,z,o):
        i=0
        while i <len(self.database):
            if (self.database[i] == RSSIRecord(x, y, z, o)):
                return i
            else:
                i=i+1
        return -1
    ...
    def importCsvFile(self, file_name):
      with open(file_name, 'r') as file:
        ...
        for row in my_reader:
          i = self.checkExistance(row[0], row[1], row[2], 0)
          if (i<0):
